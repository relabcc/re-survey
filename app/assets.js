/* eslint-disable */
export default [
  require('./components/ArrowDown/down.svg'),
  require('./components/Bubble/bubble-1-bottom.svg'),
  require('./components/Bubble/bubble-1-top.svg'),
  require('./components/Bubble/dialog-lower-flipped.svg'),
  require('./components/Bubble/dialog-lower.svg'),
  require('./components/Bubble/dialog-upper.svg'),
  require('./components/Buttons/button-1-hover.svg'),
  require('./components/Buttons/button-1.svg'),
  require('./components/Buttons/button-2-hover.svg'),
  require('./components/Buttons/button-2.svg'),
  require('./components/Buttons/button-3-hover.svg'),
  require('./components/Buttons/button-3.svg'),
  require('./components/DragMe/bubble-2.svg'),
  require('./components/DragMe/click-me.svg'),
  require('./components/FbShare/fbicon-hover.svg'),
  require('./components/FbShare/fbicon.svg'),
  require('./components/FixedLogo/title-icon.svg'),
  require('./components/Legs/legs.svg'),
  require('./components/LinkArrow/link-arrow.svg'),
  require('./components/List/bulletin-point.svg'),
  require('./components/Paper/paper-bottom.svg'),
  require('./components/Steps/timeline-1.svg'),
  require('./components/Steps/timeline-2.svg'),
  require('./components/Steps/timeline-3.svg'),
  require('./components/Steps/timeline-4.svg'),
  require('./components/Steps/timeline-5.svg'),
  require('./components/Steps/timeline-6.svg'),
  require('./components/TextWithIcon/bubble-1-head.svg'),
  require('./components/TurnMe/bubble-1.svg'),
  require('./components/Underline/line-black.svg'),
  require('./components/Underline/line.svg'),
  require('./components/Underline/underline-square-black.svg'),
  require('./components/Underline/underline-square.svg'),
  require('./containers/HomePage/dr.svg'),
  require('./containers/HomePage/relab-logo.svg'),
  require('./containers/QuizPage/confidence.svg'),
  require('./containers/QuizPage/InputModules/Clock/clock-1.svg'),
  require('./containers/QuizPage/InputModules/Clock/clock-2.svg'),
  require('./containers/QuizPage/InputModules/Clock/clock-face.svg'),
  require('./containers/QuizPage/InputModules/Clock/long hand.svg'),
  require('./containers/QuizPage/InputModules/Clock/short hand.svg'),
  require('./containers/QuizPage/InputModules/Degree/legend.svg'),
  require('./containers/QuizPage/InputModules/Pie/legend-mobile.svg'),
  require('./containers/QuizPage/InputModules/Pie/legend.svg'),
  require('./containers/QuizPage/love.svg'),
  require('./containers/QuizPage/nervous.svg'),
  require('./containers/ResultPage/double-line.svg'),
  require('./containers/ResultPage/i-want-lesson.svg'),
  require('./containers/ResultPage/Prescription/black-logo.svg'),
  require('./containers/ResultPage/Prescription/hand-overlay.svg'),
  require('./containers/ResultPage/Prescription/others.svg'),
  require('./containers/ResultPage/Prescription/three-head.svg'),
  require('./containers/ResultPage/Prescription/title.svg'),
  require('./containers/ResultPage/pyramid.svg')
];
